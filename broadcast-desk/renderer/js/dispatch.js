...
import { platform, limitFor, countFor, costOf } from "../../shared/platforms.js";
...
export function checkAccount(account, text, { hasMedia = false, live = !api.isDryRun() } = {}) {
  const pl = platform(account.platform);
  const problems = [];
  ...
  if (pl.cost && text) {
    const c = costOf(pl.id, text);
    if (c.amount) {
      notes.push(c.link
        ? `Contains a link: $${c.amount.toFixed(2)} on ${pl.name} instead of $${pl.cost.perPost.toFixed(3)}.`
        : `$${c.amount.toFixed(3)} on ${pl.name}.`);
    }
  }
...
export async function send(post, { dryRun = false } = {}) {
  ...
  const items = sendable.map((r) => ({
    ...
    target: post.target || r.account.target || null,
    ...
    allowLinkCost: !!post.allowLinkCost,
  }));
 ...
 */
export function toQueueEntries(posts, { unattendedOnly = true } = {}) {
  const entries = [];
  for (const post of posts) {
    const items = resolve(post)
      .filter((r) => r.account && r.account.connectionId && !r.problems.length)
      .filter((r) => (unattendedOnly ? r.account.unattended : true))
      .map((r) => ({
        accountId: r.account.id,
...
/**
 * The other half: posts the in-app scheduler owns, because their accounts are
 * local-only. Splitting them this way is what stops both senders taking the
 * same post and publishing it twice.
 */
export function toLocalEntries(posts) {
  const entries = [];
  for (const post of posts) {
    const items = resolve(post)
      .filter((r) => r.account && r.account.connectionId && !r.account.unattended && !r.problems.length)
      .map((r) => ({
        accountId: r.account.id,
        connectionId: r.account.connectionId,
        platform: r.account.platform,
