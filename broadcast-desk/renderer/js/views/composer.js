...
import { platform, countFor } from "../../../shared/platforms.js";
...
export const subtitle = () => api.isDryRun()
  ? "Dry run — nothing is published."
  : "Live. Posts go out for real.";
...
export function render(root, ctx) {
    ...
    <div class="split">
      <div class="stack">
        ...
        <section class="card">
          ...
          <div class="card-body">
            <div class="field">
              <label class="label">Media <span class="muted">(required for Instagram, TikTok, Pinterest and Vimeo)</span></label>
              <div class="row tight" id="mediaList"></div>
              <div class="row tight" style="margin-top:.4rem">
                <button class="btn btn-sm" data-add-media>Choose files…</button>
                ${draft.media.length ? html`<button class="btn btn-sm btn-ghost" data-clear-media>Remove all</button>` : raw("")}
              </div>
            </div>
            <div class="row">
            ...
            <div id="costLine" style="margin-top:.5rem"></div>
          </div>
        </section>
  ...
  on(ctx.scope, "click", "[data-add-media]", async () => {
    const picked = await api.pickMedia();
    if (!picked || !picked.length) return;
    const seen = new Set(draft.media.map((m) => m.path));
    draft.media = draft.media.concat(picked.filter((m) => !seen.has(m.path)));
    paintMedia(root);
    refreshDerived();
  });
  on(ctx.scope, "click", "[data-drop-media]", (e, t) => {
    draft.media = draft.media.filter((m) => m.id !== t.dataset.dropMedia);
    paintMedia(root);
    refreshDerived();
  });
  on(ctx.scope, "click", "[data-clear-media]", () => {
    draft.media = [];
    paintMedia(root);
    refreshDerived();
  });
...
/** Chips for the chosen files, with a thumbnail where one is cheap to make. */
async function paintMedia(root) {
  const box = $("#mediaList", root);
  if (!box) return;
  if (!draft.media.length) {
    mount(box, html`<span class="small muted">No media. Text-only posts are fine on X, Telegram, Discord, Reddit, Tumblr, Facebook and WordPress.</span>`);
    return;
  }
  mount(box, html`${draft.media.map((m) => html`
    <span class="chip" title="${m.path}">
      ${m.kind === "video" ? "▶" : "▣"} ${m.name}
      <span class="muted">${(m.size / 1048576).toFixed(1)} MB</span>
      <button data-drop-media="${m.id}" aria-label="Remove">✕</button>
    </span>`)}`);
}

/**
 * What this send will cost. Only X charges, and only this is where an operator
 * finds out before the money is spent rather than on a monthly invoice.
 */
async function paintCost(root, ctx) {
  const box = $("#costLine", root);
  if (!box) return;
  const accounts = ctx.accounts().filter((a) => draft.selected.has(a.id));
  const items = accounts.map((a) => ({ accountId: a.id, platform: a.platform, text: draft.body }));
  const est = await api.estimate(items);
  if (!est.total) { mount(box, raw("")); return; }

  const links = est.lines.filter((l) => l.link).length;
  mount(box, html`
    <div class="notice ${est.wouldExceed ? "bad" : links ? "" : "info"}">
      <b>$${est.total.toFixed(2)}</b> to send this
      ${links ? html` — ${plural(links, "post")} contains a link, at $0.20 each instead of $0.015` : raw("")}.
      <span class="muted">$${est.spentThisMonth.toFixed(2)} of $${est.cap} used this month.</span>
      ${est.wouldExceed ? html`<br><b>This would cross your monthly cap and will be refused.</b>` : raw("")}
    </div>`);
}
...
async function postNow(ctx) {
  ...
  const estimate = await api.estimate(resolved.map((r) => ({
    accountId: r.a.id, platform: r.a.platform, text: r.text,
  })));
  ...
  const v = await modal({
    ...
    wide: true,
    body: html`
      ${api.isDryRun() ? html`<div class="notice"><b>Dry run.</b>
        This records the post without publishing it.</div>` : raw("")}
      ${estimate.total ? html`<div class="notice ${estimate.wouldExceed ? "bad" : ""}">
        This send costs <b>$${estimate.total.toFixed(2)}</b> on X.
        ${estimate.lines.some((l) => l.link)
          ? html`<br>A link makes each X post $0.20 instead of $0.015.
                 <label class="check" style="margin-top:.4rem">
                   <input type="checkbox" name="allowLinkCost" />
                   <span>Send it anyway and accept the charge</span></label>`
          : raw("")}
        ${estimate.wouldExceed ? html`<br><b>Over the monthly cap — raise it in Settings first.</b>` : raw("")}
      </div>` : raw("")}
    ...
    actions: [
      ...
      { label: api.isDryRun() ? "Record dry run" : "Post now", value: "go", primary: true },
    ],
  });
  if (v !== "go") return;

  const allowLinkCost = Boolean((formData(document.getElementById("modal")) || {}).allowLinkCost);
...
async function scheduleDialog(ctx) {
  ...
  const v = await modal({
    ...
    body: html`
      ...
      ${accounts.every((a) => a.unattended) || !api.relayConfigured()
        ? html`<div class="notice">Accounts not marked <b>unattended</b> only send while
            Broadcast Desk is running. Mark them on the Connections screen to have the
            relay send them when this machine is off.</div>`
        : raw("")}`,
