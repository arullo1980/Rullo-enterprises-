...
import { health, isDryRun, isDesktop } from "./api.js";
...
async function updateConnState() {
  ...
  if (isDryRun()) {
    ...
    badge.title = "Sends are simulated. Connect an account, or switch it off in Settings.";
    return;
  }
  const h = await health();
  if (h.mode === "live") {
    badge.textContent = "live + relay";
    badge.className = "badge ok";
    badge.title = "Sending for real; the relay can post while this machine is off.";
  } else if (h.mode === "unauthorised") {
    badge.textContent = "relay key";
    badge.className = "badge bad";
    badge.title = h.error || "";
  } else if (h.mode === "error") {
    ...
    badge.className = "badge ok";
    badge.title = "Sending for real from this machine. No relay, so scheduled posts need the app running.";
  }
}
...
/**
 * Events coming the other way across the bridge: menu and tray navigation,
 * and — the important one — results from the scheduler that runs in the main
 * process, which sends whether or not this window is open.
 */
function wireDesktop() {
  if (!isDesktop()) return;
  const desk = window.desk;

  desk.onNavigate((view) => go(view));

  desk.onMenu(async (action) => {
    if (action === "export") {
      const { exportAll } = store;
      const api = await import("./api.js");
      await api.saveFile(`broadcast-desk-${new Date().toISOString().slice(0, 10)}.json`,
        JSON.stringify(exportAll(), null, 2));
      toastLike("Exported");
    }
    if (action === "import") go("settings");
  });

  desk.onScheduler(async (payload) => {
    if (!payload || payload.type !== "sent") return;
    const post = store.get("posts", payload.postId);
    if (!post) return;
    const ok = (payload.results || []).filter((r) => r.ok).length;
    await store.put("posts", {
      ...post,
      status: ok === payload.results.length ? "sent" : ok ? "partial" : "failed",
      sentAt: payload.at,
      results: payload.results,
    });
    await store.log("publish", `Scheduler sent ${ok}/${payload.results.length} for a queued post`, { postId: post.id });
    if (location.hash.startsWith("#/queue") || location.hash.startsWith("#/dashboard")) paint(location.hash);
  });
}

/** The toast helper lives in ui.js; this avoids a cycle at module load. */
function toastLike(message) {
  import("./ui.js").then((ui) => ui.toast(message, "ok"));
}
