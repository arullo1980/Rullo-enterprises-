...
import { send, resolve, toQueueEntries } from "../dispatch.js";
...
export const actions = () => `
  <button class="btn" data-run>${api.isLive() ? "Sync with backend" : "Send due now"}</button>
...
export function render(root, ctx) {
  ...
  mount(root, html`
    ${api.isLive()
      ? html`<div class="notice info" style="margin-bottom:1rem">
          <b>The Worker owns the schedule.</b> Queued posts are handed over and sent by
          its cron, so they go out whether or not this is open. This tab does not
          send, to rule out the same post going twice.
        </div>`
      : html`<div class="notice" style="margin-bottom:1rem">
          <b>This tab is the scheduler.</b> With no backend configured, due posts are
          sent from here while the tab is open — and simulated, not published.
          Deploy the <code>social-hub</code> Worker for unattended sending.
        </div>`}
  ...
  on(ctx.scope, "click", "[data-run]", () => (api.isLive() ? syncBackend(ctx, true) : runDue(ctx, true)));

  startRunner(ctx);
  if (api.isLive()) syncBackend(ctx, false);
}
...
/**
 * Send anything that has come due. Kept deliberately serial: a burst of
 * simultaneous posts from one desk is exactly the pattern the networks
 * rate-limit on.
 */
...
function startRunner(ctx) {
  if (runner) clearInterval(runner);
  runner = setInterval(() => {
    if (!location.hash.startsWith("#/queue")) { clearInterval(runner); runner = null; return; }
    // With a backend, the cron is the only thing that sends; this only collects
    // what it did. Sending from both places is how a post goes out twice.
    if (api.isLive()) syncBackend(ctx, false);
    else if (due(store.all("posts")).length) runDue(ctx, false);
  }, 30000);
}

/**
 * Hand the queue to the Worker and collect what its cron has already sent.
 * Posts whose accounts are not connected stay here rather than being handed
 * over, so nothing sits in a remote queue that can never be fulfilled.
 */
async function syncBackend(ctx, loud) {
  ...
  try {
    ...
    const unsendable = queued.length - entries.length;
    ...
    if (loud) {
      toast(`Synced ${r.synced} of ${queued.length} queued` +
            (unsendable ? `, ${unsendable} need a connected account` : "") +
            (applied ? ` · ${applied} sent by the Worker` : ""), "ok", 5000);
    }
    if (applied) ctx.refresh();
  } catch (e) {
    if (loud) toast("Sync failed: " + e.message, "bad", 5000);
  }
}
